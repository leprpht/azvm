...
var inspectCmd = &cobra.Command{Use: "inspect <vm>", Short: "Show VM compute and network information", Args: cobra.ExactArgs(1), RunE: func(cmd *cobra.Command, args []string) error {
...
func formatReport(r azure.VMReport) string {
	...
	for _, n := range r.Network {
		fmt.Fprintf(&b, "  NIC:             %s\n  VNet:            %s\n  Subnet:          %s\n  Private IP:      %s\n  Public IP:       %s\n", n.NIC, n.VNet, n.Subnet, n.PrivateIP, n.PublicIP)
	}
	return b.String()
}
